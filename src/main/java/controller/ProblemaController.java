...
public class ProblemaController {
    ...
    public DefaultTableModel obtenerTablaProblemas() {
        String[] columnas = {"ID Problema", "Fecha Inicio", "Fecha Fin", "Estado", "ID Cliente", "Descripcion"};
        DefaultTableModel modelo = new DefaultTableModel(null, columnas);
        List<Problema> lista =  problemaDAO.obtenerTodos();
        for (Problema c : lista) {
            modelo.addRow(new Object[]{c.getIdProblema(), c.getFchIni(), c.getFchFin(),c.getEstado(), c.getIdCliente(), c.getDescripcion()});
        }
        return modelo;
    }
